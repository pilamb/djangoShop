...
urlpatterns = \
    patterns(
        ...
        url(r'^createUser$','theCode.views.create_user2.page',
            name='create_user'),
        url(r'^listUsers/',UserModelListView.as_view(), name='users_list'),
        # only ADMIN, for panel
        url(r'^detailUser/(?P<pk>\d+)/$', UserModelDetailView.as_view(),
            name='detail_user'),
        url(r'^editUser/(?P<pk>\d+)/$', UserModelUpdateView.as_view(),
            name='edit_user'),
        url(r'^deleteUser/(?P<pk>\d+)/$', UserModelDeleteView.as_view(),
            name='delete_user'),
        ...
        url(r'^createOrder2/(?P<pk>\d+)/$',
            ...
            name='create_order2'),  #se viene of haber elegido ya un modulo, pk
        url(r'^listOrders/',
            OrderListView.as_view(),
            name='listado_orders'),  #soloADMIN, para panel
        url(r'^detailOrder/(?P<pk>\d+)/$',
            OrderDetailView.as_view(), name='detail_order'),
        url(r'^EditOrder/(?P<pk>\d+)/$',
            OrderUpdateView.as_view(),name='Edit_order'),
        url(r'^deleteOrder/(?P<pk>\d+)/$',
            OrderDeleteView.as_view(),name='delete_order'),
        url(r'^OrdersUser/',
            OrdersUserModelListView.as_view(),name='ordersUser'),

        url(r'^detailSale/(?P<pk>\d+)/$',
            SaleDetailView.as_view(), name='detail_sale'),
        url(r'^chart_sales$',
        ...
        url(r'^notification/(?P<pk>\d+)/$',
            NotificationDetailView.as_view(), name='detail_notificacion'),
        url(r'^messages/(?P<pk>\d+)/$',
            NotificationListView.as_view(), name='messages'),
        url(r'^listMessage_classes/$',
            NotificationListView.as_view(), name='ListaMessage_classes'),
        ...
        url(r'^detailShipment/(?P<pk>\d+)/$',
            ShipmentDetailView.as_view(),name='detail_shipment'),
        url(r'^invoice/(?P<pk>\d+)/$',
            InvoicePDFModel.as_view(), name='imprimir_invoice'),
