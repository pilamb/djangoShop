...
class MessageDetailView(LoginRequiredMixin,DetailView):
	...
	def get_object(self):
			...
			#Marcar como leido el message para que el admin pueda verlo
			object.visto()
			object.save()
			return object
...
class MessageListView(LoginRequiredMixin,ListView):
	...
	def get_connotified_data(self, **kwargs):
			connotified = super(MessageListView, self).get_connotified_data(**kwargs)  
			return connotified	
	def get_queryset(self):
		if not self.request.user.is_admin:
			return Message.objects.filter(user = self.request.user)
		else:
			return Message.objects.all()#filtrar por recientes
